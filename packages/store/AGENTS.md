...
| e | Every execution (`exec`, `run`, `get`, `all`; `prepare` alone does not) leaks a GWeakRef, and the budget is per PROCESS: ~65 000 executions, after which every SELECT returns `[]` — see (a). Closing the connection and `System.gc()` reclaim nothing (gjsify gap, 0.49.0, unfixed, gjsify fix/sqlite-weakref-leak). `run()` counts three: it also selects `changes()` and `last_insert_rowid()` | **Count executions.** Bulk writes go through `insertMany` (multi-row, 120 bound values a statement — parse cost grows with the square of the parameter count) and set-based `uid IN (…)` chunks; never one `run()` per message. The 6 000-message resync test in `sync.test.ts` fails on per-row writes. |
